...
def do_sim(args):
    ...
    while env.viewer.is_running():
        ...
        for i in range(len(joint_commands)):
            joint_commands[i] = axis_direction[i] * (positions[i] - start_pos[i]) * counts_to_radians + offsets[i]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, default="/dev/ttyACM0", help="Port name (e.g., COM1, /dev/ttyUSB0, /dev/tty.usbserial-*)"
    )
